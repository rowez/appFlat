...
if(isset($_POST['email'])) {
  ...
  if(!preg_match($string_exp,$meisjesnaam)) {
    ...
    if(!preg_match($string_exp,$partner)) {
  	$error_message_nm .= 'Je hebt geen juiste partner ingevoerd.<br />';
  }
	...
	if(!preg_match($string_exp,$postcode)) {
  	$error_message .= 'Je hebt geen juiste postcode ingevoerd.<br />';
  }
      $string_exp = '/^((\p{L}\p{M}*)|(\p{Cc})|(\p{P}\p{P}\p{N}.{3})|(\p{N})|(\p{Zs}))+$/ui';
	if(!preg_match($string_exp,$woonplaats)) {
  	$error_message .= 'Je hebt geen juiste woonplaats ingevoerd.<br />';
  }
  ...
  if(strlen($comments) < 2) {
  	$error_message_nm .= 'The Comments you entered do not appear to be valid.<br />';
